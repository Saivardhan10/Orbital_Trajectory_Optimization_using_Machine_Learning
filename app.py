...
if uploaded_input and uploaded_features:
    ...
    try:
        ...
        # Create the orbit with the predicted parameters
        orbit = Orbit.from_classical(
            Earth,
            semi_major_axis,
            eccentricity,
            inclination,
            0 * u.deg,  # Right Ascension of Ascending Node (RAAN)
            0 * u.deg,  # Argument of Periapsis
            0 * u.deg,  # True Anomaly
        )

        # Extract the orbit positions for plotting
        time_of_flight = np.linspace(0, 1, 500) * orbit.period  # Full orbit period
        ...
        fig.update_layout(
            ...
            margin=dict(l=0, r=0, b=0, t=40),  # Adjust the margin for better visualization
            showlegend=True
        )
